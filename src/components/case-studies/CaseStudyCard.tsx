...
export default function CaseStudyCard({ caseStudy }: CaseStudyCardProps) {
  return (
    <Link href={`/case-studies/${caseStudy.slug}`} className="group block h-full">
      <motion.div
        className="h-full bg-white rounded-2xl overflow-hidden border border-[var(--color-border)] flex flex-col"
        whileHover={{ y: -6, boxShadow: '0 20px 48px rgba(1,7,56,0.13)' }}
        transition={{ duration: 0.25, ease: 'easeOut' }}
      >
        {/* Logo */}
        <div className="px-6 pt-6 pb-4 min-h-[56px] flex items-center">
          {caseStudy.logo ? (
        ...
        {/* Hero image */}
        {caseStudy.heroImage && (
          <div className="relative w-full aspect-[16/9] overflow-hidden bg-[var(--color-surface-muted)]">
            <Image
